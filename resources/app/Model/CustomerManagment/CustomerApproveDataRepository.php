...
class CustomerApproveDataRepository extends EloquentRepositoryAbstract {
    ...
    {
        $customer = Customer::select('customer.id','business_name', 'owner_name','customer.email', 'contact_no','alernate_no','country_id','city_id','address','user_id', 'cnic', 'bank_name','branch_name','account_no','account_title','brith_date','anniversary_date');
    ...
    public function getRows($limit, $offset, $orderBy = null, $sord = null, array $filters = array(),$nodeId = null, $nodeLevel = null,$exporting)
    {
        ...
        ->select('customer.id','business_name','users.status',  'owner_name','customer.email', 'contact_no','alernate_no','country_id','city_id','address','user_id', 'cnic', 'bank_name','branch_name','account_no','account_title','brith_date','anniversary_date');
        ...
        if(!empty($customer)){
            ...
            foreach($customer as $cus) {
                ...
                if($cus->status == 0){
                    ...
                    $EditBtn = "<a onclick=editmaintenancerequest('".$actionUrldisable."') class='btn-cus-dessign btn btn-primary waves-effect waves-light'>Block</a> ";

                }

                $data[] = array(
                    'id'			=>	$cus->id,
                    'business_name'			=>	$cus->business_name,
                    'owner_name'			=>	$cus->owner_name,
                    'email'			=>	$cus->email,
